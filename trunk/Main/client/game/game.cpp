...
void CGame::StartGame(BOOL NewspapersDisabled)
{
	DWORD dwVP, dwVP2;
	...
	*(BYTE *)0x4F6538 = 0x75;
	...
	strcpy((PCHAR)0x6886AC,"jmnvc.scm");	
	VirtualProtect((PVOID)0x6886AC,9,dwVP,&dwVP2);

	// Patch to modify the scm path
	VirtualProtect((PVOID)0x6D7368,14,PAGE_EXECUTE_READWRITE,&dwVP);
	strcpy((PCHAR)0x6D7368,"data\\jmnvc.scm");	
	VirtualProtect((PVOID)0x6D7368,14,dwVP,&dwVP2);

	// Patch to modify the map path
	//VirtualProtect((PVOID)0x68E5A1,14,PAGE_EXECUTE_READWRITE,&dwVP);
	//strcpy((PCHAR)0x68E5A1,"jmnvc_map.dat");	
	//VirtualProtect((PVOID)0x68E5A1,14,dwVP,&dwVP2);

	/* Patch to modify the gxt path
	VirtualProtect((PVOID)0x69A4B0,256,PAGE_EXECUTE_READWRITE,&dwVP);
	strcpy_s((PCHAR)0x69A4B0,"vcmp.gxt");
	strcpy_s((PCHAR)0x69A4C0,"vcmp.gxt");
	strcpy_s((PCHAR)0x69A4CC,"vcmp.gxt");
	strcpy_s((PCHAR)0x69A4D8,"vcmp.gxt");
	strcpy_s((PCHAR)0x69A4E4,"vcmp.gxt");
	...
	*(BYTE *)0x4F6A22 = 0x00;
	...
	// Hack to prevent rubbish and newspapers
	if (NewspapersDisabled==TRUE){
		VirtualProtect((PVOID)0x567F20,1,PAGE_EXECUTE_READWRITE,&dwVP);
		*(BYTE *)0x567F20 = 0xC3;
		VirtualProtect((PVOID)0x567F20,1,dwVP,&dwVP2);
	}
