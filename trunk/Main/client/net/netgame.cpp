...
{
	//strcpy(szPlrNameTemp,szPlayerName);
	//iPortTemp = iPort;
...
void CNetGame::Reconnect(void)
{
	...
	m_pPlayerPool = new CPlayerPool();
	m_pPlayerPool->SetLocalPlayerName(szPlrNameTemp);
	m_pVehiclePool = new CVehiclePool();
	m_pRakClient->Connect(szHostTemp,iPortTemp,(iPortTemp+1),0,5);*/
}
